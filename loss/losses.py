...
class SupConLoss(nn.Module):
    ...
    def forward(self, *views, labels=None, mask=None):
        assert len(views) == 2, "the number of views must be = 2"
        # image3d_feature, graph_feature = views[0], views[1]
        features = get_forward_features_from_multi_view(*views)  # shape [bsz, n_views, ...].
